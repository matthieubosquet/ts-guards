...
arrayTestArray.forEach((arrayTest) => {
  ...
  test(`isArrayOf(${String(arrayTest.value)}, isString) returns ${
    arrayTest.type === "string"
  }`, () => {
    expect(
      standardObject.isArrayOf(arrayTest.value, primitiveType.isString)
    ).toBe(arrayTest.type === "string");
  });
});
